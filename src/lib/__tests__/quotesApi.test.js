...
import { makeQuotesApi } from "../quotesApi.js";
...
const quotesApi = makeQuotesApi("marine-transportation");
...
describe("quotesApi", () => {
  it("list() GETs /api/units/:unit/quotes and returns the parsed JSON", async () => {
    global.fetch.mockResolvedValue(jsonResponse([{ id: "1" }]));
    const result = await quotesApi.list();
    expect(global.fetch).toHaveBeenCalledWith("/api/units/marine-transportation/quotes");
  ...
  it("update() PATCHes the specific quote", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith("/api/units/marine-transportation/quotes/1", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  it("remove() DELETEs the specific quote", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith("/api/units/marine-transportation/quotes/1", { method: "DELETE" });
  });

  it("clearAll() DELETEs the whole register", async () => {
    global.fetch.mockResolvedValue(jsonResponse(null, 204));
    await quotesApi.clearAll();
    expect(global.fetch).toHaveBeenCalledWith("/api/units/marine-transportation/quotes", { method: "DELETE" });
  });
  ...
  it("scopes different units to different URLs", async () => {
    global.fetch.mockResolvedValue(jsonResponse([]));
    await makeQuotesApi("retail").list();
    expect(global.fetch).toHaveBeenCalledWith("/api/units/retail/quotes");
  });
});
