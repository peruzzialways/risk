...
const ROW = {
  id: "1", insured: "Alpha Mills", broker: "Crownfield", officer: "Ada Okafor", risk_class: "Fire only",
...
describe("rowToQuote", () => {
  it("maps snake_case DB fields to camelCase and coerces numeric strings", () => {
    expect(rowToQuote(ROW)).toEqual({
      id: "1", insured: "Alpha Mills", broker: "Crownfield", officer: "Ada Okafor", riskClass: "Fire only",
      month: "Jan", year: 2026, sumInsured: 100000000, premium: 1000000,
      status: "Incepted", roComment: "Bound", createdAt: "2026-01-01T00:00:00Z",
    });
  });

  it("defaults missing broker/officer/comment to empty strings", () => {
    const q = rowToQuote({ ...ROW, broker: null, officer: null, ro_comment: null });
    expect(q.broker).toBe("");
    expect(q.officer).toBe("");
...
describe("quoteToRow", () => {
  it("maps a full camelCase quote to a snake_case row", () => {
    expect(
      quoteToRow({
        insured: "Alpha Mills", broker: "Crownfield", officer: "Ada Okafor", riskClass: "Fire only",
        month: "Jan", year: 2026, sumInsured: 100000000, premium: 1000000,
        status: "Incepted", roComment: "Bound",
      })
    ).toEqual({
      insured: "Alpha Mills", broker: "Crownfield", officer: "Ada Okafor", risk_class: "Fire only",
      month: "Jan", year: 2026, sum_insured: 100000000, premium: 1000000,
